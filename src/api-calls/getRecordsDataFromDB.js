...
export default (queryObj) => {
  ...
  const records = mockDB.filter((record) => {
    ...
    const demandBreakfast = queryWithBreakfast ? withBreakfast : withBreakfast ? false : true
    const demandLodging = queryWithLodging ? withLodging : withLodging ? false : true
